...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <label for="fid">favorite_id:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO favorites(favorite_id, id, artwork_id) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $fid, $uid, $arid);
      ...
      include('database_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $fid = $row['favorite_id']; // Fetch the favorite_id
                echo "<tr>
                    <td>" . $row['favorite_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['artwork_id'] . "</td>
                    <td><a style='padding:4px' href='delete_favorites.php?favorite_id=$fid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_favorites.php?favorite_id=$fid'>Update</a></td> 
                </tr>";
            }
        } else {
