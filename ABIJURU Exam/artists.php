...
<section>
    ...
    <form method="post" onsubmit="return confirmInsert();">
        ...
        <input type="text" id="sm" name="sm" required><br><br>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO artists(artist_id, id, biography, social_media) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $aid, $uid, $bio, $sm);
...
// SQL query to fetch data from the artists table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of artists</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of artists</h2></center>
    <table border="5">
        <tr>
            <th>artist_id</th>
            <th>id</th>
       ...
       include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all artists
        $sql = "SELECT * FROM artists";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $aid = $row['artist_id']; // Fetch the artist_id
                echo "<tr>
                    <td>" . $row['artist_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['biography'] . "</td>
                    <td>" . $row['social_media'] . "</td>
                    <td><a style='padding:4px' href='delete_artists.php?artist_id=$aid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_artists.php?artist_id=$aid'>Update</a></td> 
                </tr>";
            }
        } else {
