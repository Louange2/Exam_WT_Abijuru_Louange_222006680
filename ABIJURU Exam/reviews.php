...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO reviews(review_id, id, artwork_id, rating) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $rid, $uid, $arid, $rt);
...
// SQL query to fetch data from the reviews table
      ...
      include('database_connection.php');
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $rid = $row['review_id']; // Fetch the review_id
                echo "<tr>
                    <td>" . $row['review_id'] . "</td>
                    <td>" . $row['id'] . "</td>
                    <td>" . $row['artwork_id'] . "</td>
                    <td>" . $row['rating'] . "</td>
                    <td><a style='padding:4px' href='delete_reviews.php?review_id=$rid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_reviews.php?review_id=$rid'>Update</a></td> 
                </tr>";
            }
        } else {
