...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO exhibitions(exhibition_id, exhibition_name, location, gallery_id) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $eid, $enm, $loc, $gid);
...
// SQL query to fetch data from the exhibitions table
...
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>Detail information Of exhibitions</title>
    <style>
        table {
            width: 100%;
...
<body>
    <center><h2>Table of exhibitions</h2></center>
    <table border="5">
        <tr>
            <th>exhibition_id</th>
     ...
     include('database_connection.php');
        ...
        // Prepare SQL query to retrieve all exhibition
        $sql = "SELECT * FROM exhibitions";
        $result = $connection->query($sql);
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                $eid = $row['exhibition_id']; // Fetch the exhibition_id
                echo "<tr>
                    <td>" . $row['exhibition_id'] . "</td>
                    <td>" . $row['exhibition_name'] . "</td>
                    <td>" . $row['location'] . "</td>
                    <td>" . $row['gallery_id'] . "</td>
                    <td><a style='padding:4px' href='delete_exhibitions.php?exhibition_id=$eid'>Delete</a></td> 
                    <td><a style='padding:4px' href='update_exhibitions.php?exhibition_id=$eid'>Update</a></td> 
                </tr>";
            }
        } else {
